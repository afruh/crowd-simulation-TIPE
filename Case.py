...
class case :
    def __init__(self, pos = vect2D(), dim = 0, type = 0, score = [inf,inf], canvas = "", color = "ivory", explore = False, grille = False):
        self.pos = pos          # Position de la case
        self.dim = dim          # Dimension de la case en pixels
        self.type = type        # Type de la case
        self.score = score     # Distance du plus court chemin à la sortie (1,2)
    ...
    def degrade(self, fg, bg, maxd):
        ...
        col = blend(self.score[Var.choixSortie-1] / maxd,fg, bg)
        if(self.grille):
...
def init_case(terrain):
    ...
    for x in range(Var.largeur):
        for y in range(Var.hauteur):
            pos = vect2D(x, y)
            c = case(pos, Var.dimCase, 0, [inf,inf], terrain, "ivory", False)
            Var.TCase[y, x] = c
    return
